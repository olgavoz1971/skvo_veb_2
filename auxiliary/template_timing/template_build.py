...
from manifest_config import GPTemplateDefaults
...
def fit_gp_template(frag: pd.DataFrame, cfg: GPTemplateDefaults, *, fold_period: float) -> dict:
    ...
    tau_peak = select_template_peak_tau(
        ...
        local_order=cfg.peak_local_order,
        period_pair_tol=cfg.peak_pair_period_tol,
        min_prominence_frac=cfg.peak_min_prominence_frac,
    )
    tau_mask_min, tau_mask_max = fit_tau_mask(
        tau_peak,
        length_scale_final,
        half_width_factor=cfg.mask_length_scale_factor,
        min_half_width=cfg.mask_min_half_width,
    )
...
def plot_template_fit(
    ...
    *,
    ...
    """Matplotlib: folded data and GP mean."""
    apply_plot_style()
    x = result["x"]
...
def build_piece_template(
    ...
    *,
    ...
    plot_template_fit(result, cfg, save_path=out_plot, show=show_plot)
    return result
