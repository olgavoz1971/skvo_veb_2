...
class FluxColumnSpec:
    ...
    # -- background columns, mirroring the flux fields above -----------------
    bkg_col: Optional[str] = None       # background column name, None if not delivered at all
    bkg_err_col: Optional[str] = None   # None if no dedicated error column is provided
    background_type: Optional[str] = None
        # "physical"     -- a genuine, always non-negative sky/aperture background
        #                   estimated from the calibrated frame itself (SPOC/Kepler
        #                   convention: SAP_BKG in e-/s)
        # "differential" -- measured on a *difference image* (frame minus reference
        #                   template); represents how far the local background has
        #                   drifted relative to the template, and can legitimately
        #                   be negative (QLP convention)
        # None           -- not delivered as a per-cadence light-curve column at all
        #                   (e.g. TGLC subtracts background upstream, at the pixel/
        #                   PSF-fitting stage, and does not expose it here)
    bkg_notes: str = ""
