"""Detrending algorithms for the standalone trend probe.

Rewritten from ``plot_tcp_lc.py`` so the sandbox file stays untouched. The LSQ
spline accepts an explicit interior-knot vector for mouse editing.
"""
...
def apply_detrend_method(
    ...
    *,
    ...
    """Fit one trend method and return trend, residual, and residual errors.
    ...
    Returns:
        tuple: ``(trend, detrended, detrended_err)``.
    ...
    detrended = detrend_observed(values, trend, mode)
    if values_err is None:
        det_err = np.full(values.shape, np.nan, dtype=float)
    else:
        det_err = detrended_standard_error(values, trend, values_err, mode)
    logger.info("Fitted %s trend on %s points", method_id, len(times))
    return trend, detrended, det_err
