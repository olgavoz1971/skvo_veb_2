"""Plotly figures for the detrend probe (data in, figures out)."""
...
# Same type as the GP prep plot (plotly_white + Open Sans). Tight margins; no title.
_PLOT_FONT = dict(
    family=(
...
def _jd_to_plot_x(jd_values, time_axis_mode: str, display_epoch: float):
    """Map absolute Julian Dates onto the plot x-axis.
...
def _error_y(obs_err: np.ndarray | None, show_errors: bool) -> dict | None:
    """Build GP-style error bars when uncertainties are present and requested.
...
def _point_label_name(value) -> str | None:
    """Return a legend name for one label cell, or ``None`` if unmarked.

    Missing values (``None``, NaN, empty, the string ``None``) are not plotted
    as a labelled group.
...
def _legend_sort_key(name: str) -> tuple:
    """Sort legend entries numerically when the name is a number.
...
def _apply_label_legend(fig: go.Figure, *, show: bool) -> None:
    """Place a Plotly legend outside the plotting area, or hide it.
...
def _add_photometry_traces(
    ...
    *,
    ...
    """Add photometry markers, coloured by label when labels are present.

    Unlabelled / ``None`` points stay GP-blue and do not appear in the legend.
    ``customdata`` holds ``perm_index`` so Discovery-style click / lasso
    selection can mark rows on the working ``CurveDash``.

    Args:
        fig (plotly.graph_objects.Figure): Target figure.
        x_mjd: Plot x coordinates (display MJD floats, or datetimes in Date mode).
    ...
    _add_group(unmarked, str(_PHOT_MARKER["color"]), default_name, False)
    show_legend = len(marked) > 1
    for i, name in enumerate(marked):
        mask = np.array([item == name for item in names], dtype=bool)
        _add_group(mask, _LABEL_COLORS[i % len(_LABEL_COLORS)], name, show_legend)
    return show_legend


def _apply_y_axis_direction(fig: go.Figure, *, invert_y: bool) -> None:
    """Set magnitude vs flux y-axis direction the same way as the GP page.

    Magnitudes are drawn reversed (brighter up). Flux is not reversed.
    Domain belongs in the figure ``uirevision``, so Plotly resets the y-range
    when the working domain changes.
...
def _empty_figure(
    *,
    ...
    """Return a placeholder figure before a light curve is loaded.
...
def merge_knot_drag(
    ...
    *,
    ...
    """Apply shape-drag updates onto an existing knot list.

    Knots are stored as absolute JD. Shape coordinates from the plot are
    converted back before the list is updated.

    Args:
        knots (list[float]): Current interior knot times (absolute JD).
        relayout_data (dict | None): Dash ``relayoutData`` payload.
        display_epoch (float): MJD display origin (same as the plot axis).
        time_axis_mode (str): ``mjd`` or ``date``.

    Returns:
        list[float] | None: Sorted unique knots (JD) if a drag happened; else ``None``.
    """
    ...
    for idx, x_val in updates.items():
        if 0 <= idx < len(new_knots) and np.isfinite(x_val):
            new_knots[idx] = x_val
    unique = sorted({float(k) for k in new_knots if np.isfinite(k)})
    return unique
...
def extract_xaxis_range_mjd(relayout_data: dict | None) -> list[float] | None:
    """Read the plot x-axis range (display MJD) from Plotly ``relayoutData``.

    Args:
        relayout_data (dict | None): Dash ``relayoutData`` payload.

    Returns:
        list[float] | None: ``[xmin, xmax]`` in display MJD; an empty list when
        the axis was reset to autorange; ``None`` when the payload has no
        x-range update.
    """
...
def knot_hit_span_jd(
    ...
    *,
    ...
    """Return the time span used for delete-knot proximity.

    Prefers the current plot x-range. Falls back to the full series span
    when the view has not been zoomed.
...
def knot_click_edit(
    ...
    *,
    ...
    """Add or delete an interior knot according to the explicit click mode.
    ...
    Args:
        ...
        mode (str): ``add`` places a knot at the click; ``delete`` removes the
            nearest knot only if it lies within ``hit_frac`` of ``hit_span``.
        hit_span (float | None): Time window for delete proximity (days).
            Defaults to ``t_max - t_min``.
...
def _knot_y_span(y_obs: np.ndarray) -> tuple[float, float]:
    """Return a vertical span that covers the observed photometry.

    Args:
        y_obs (numpy.ndarray): Observed y values.

    Returns:
        tuple[float, float]: Inclusive ``(y_lo, y_hi)`` used to draw knot lines.
    """
...
def _add_knot_traces(
    ...
    *,
    ...
    """Draw thin non-interactive knot lines (used while selecting points).

    Args:
        fig (plotly.graph_objects.Figure): Target figure.
        knots (list[float]): Interior knot times (absolute JD).
        y_lo (float): Line start in data y.
        y_hi (float): Line end in data y.
        display_epoch (float): MJD display origin.
        time_axis_mode (str): ``mjd`` or ``date``.
    """
    for knot in knots:
        knot_x = _jd_to_plot_x(float(knot), time_axis_mode, display_epoch)
        fig.add_trace(
            go.Scatter(
                x=[knot_x, knot_x],
                y=[y_lo, y_hi],
...
def figure_raw_with_trend(
    ...
    *,
    ...
    """Build the observed-plus-trend figure, with optional knot markers.

    ``times`` and ``knots`` remain absolute JD at the call boundary.
    ...
    Args:
        ...
        invert_y (bool): Invert the y-axis (magnitudes only; flux stays upright).
        show_errors (bool): Draw error bars when uncertainties exist.
        ...
        labels: Optional per-point group labels (legend uses the raw text).
        source_index (numpy.ndarray | None): Permanent indices for clicks.
        selected_perm_indices: Permanent indices marked ``selected=1``.
        time_axis_mode (str): ``mjd`` or ``date``.

    Returns:
        plotly.graph_objects.Figure: Raw + trend plot.
    """
    axis = normalize_time_axis_mode(time_axis_mode)
...
def figure_detrended(
    ...
    *,
    ...
    """Build the detrended-residual figure.

    ``times`` is absolute JD.

    Args:
        times (numpy.ndarray | None): Observation times (absolute JD).
        detrended (numpy.ndarray | None): Detrended photometry.
        det_err (numpy.ndarray | None): Residual uncertainties.
        y_label (str): Y-axis label.
        ...
        refposition (str | None): TIMESYS refposition for the axis suffix.
        labels: Optional per-point group labels (legend uses the raw text).
        source_index (numpy.ndarray | None): Original unpack indices for clicks.
        time_axis_mode (str): ``mjd`` or ``date``.

    Returns:
        plotly.graph_objects.Figure: Residual plot with a dashed null line
        (y = 0 in magnitude, y = 1 in flux).
    """
    axis = normalize_time_axis_mode(time_axis_mode)
    xaxis_title = time_axis_xaxis_title(axis, timescale, refposition)
    if times is None or detrended is None:
        return _empty_figure(
            xaxis_title=xaxis_title,
            invert_y=invert_y,
            uirevision=uirevision,
    ...
    show_legend = _add_photometry_traces(
        ...
        np.asarray(detrended, dtype=float),
        det_err,
        labels=labels,
        source_index=source_index,
        show_errors=show_errors,
...
def graph_config(
    *,
    ...
    """Return ``dcc.Graph`` config for the current knot tool.

    Lasso and box select stay on the mode bar (same as Discovery). Knot
    shapes are draggable only in Add knot with the least-squares spline.

    Args:
        plot_tool (str): ``off``, ``add``, or ``delete``.
        method (str | None): Active detrend method id.
