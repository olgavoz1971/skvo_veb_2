"""Sliding centred-parabola smooth on unfolded calendar time.

Ported from ``auxiliary/running_parabola_spike/running_parabola.py``.
The native product is the parabola value ``a`` at each window centre.
Observation-time mapping lives in ``smooth.running_parabola_trend``.
"""
...
def window_centres(
    ...
    *,
    ...
    """Returns window-centre times that keep the full window inside the span.
...
def _fit_window_parabola(
    ...
    *,
    ...
    """Fits ``y = a + b·(t−t_c) + c·(t−t_c)²`` and returns the centre value.
...
def window_is_two_sided(t_in: np.ndarray, t_centre: float) -> bool:
    """Returns whether the in-window times have points on both sides of the centre.

    A point exactly at ``t_centre`` counts for neither side.

    Args:
        t_in (numpy.ndarray): Times inside the window (absolute JD).
        t_centre (float): Window centre.

    Returns:
        bool: ``True`` when at least one sample is strictly before and one
        strictly after the centre.
    """
    t_in = np.asarray(t_in, dtype=float)
    return bool(np.any(t_in < t_centre) and np.any(t_in > t_centre))


def evaluate_at_centre(
    jd: np.ndarray,
    phot: np.ndarray,
    phot_err: np.ndarray,
    t_centre: float,
    *,
    ...
    """Returns the parabola value at one centre using the configured window.

    Args:
        jd (numpy.ndarray): Observation times (absolute JD), already cropped.
        phot (numpy.ndarray): Photometry aligned with ``jd``.
        phot_err (numpy.ndarray): Uncertainties (may be NaN).
        t_centre (float): Window centre (absolute JD).
        cfg (RunningParabolaConfig): Window and weight settings.

    Returns:
        float: Parabola value at ``t_centre``.

    Raises:
        ValueError: If the window is too sparse, one-sided, or the fit fails.
    """
    t_all = np.asarray(jd, dtype=float)
    y_all = np.asarray(phot, dtype=float)
    e_all = np.asarray(phot_err, dtype=float)
    half = 0.5 * cfg.window_width_d
    mask = (t_all >= t_centre - half) & (t_all <= t_centre + half)
    n_in = int(np.count_nonzero(mask))
    if n_in < cfg.min_points:
        raise ValueError(
            f"window at JD {t_centre:.8f} has {n_in} point(s); "
            f"need at least {cfg.min_points}"
        )
    if not window_is_two_sided(t_all[mask], t_centre):
        raise ValueError(
            f"window at JD {t_centre:.8f} is one-sided "
            "(need points on both sides of the centre)"
        )
    smooth, _curv, _rms, _n = _fit_window_parabola(
        t_all[mask],
        y_all[mask],
        e_all[mask],
        t_centre=float(t_centre),
...
def smooth_running_parabola(
    ...
    *,
    ...
    """Builds a smoothed curve by sliding centred parabolas along calendar time.
    ...
    for t_c in centres:
        ...
        if n_in < cfg.min_points:
            skipped += 1
            continue
        if not window_is_two_sided(t_all[mask], float(t_c)):
            logger.debug("skip one-sided centre jd=%.8f", t_c)
            skipped += 1
            continue
        try:
