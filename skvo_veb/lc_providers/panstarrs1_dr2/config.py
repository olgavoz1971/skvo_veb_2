...
def band_spec_for_code(filter_code: str) -> Ps1BandSpec:
    """Returns band metadata for a filter code.

    Args:
        filter_code (str): One of ``g``, ``r``, ``i``, ``z``, ``y``.

    Returns:
        Ps1BandSpec: Matching band specification.

    Raises:
        ValueError: When ``filter_code`` is unknown.
    """
    code = str(filter_code).strip().lower()
    for band in PS1_BANDS:
        if band.filter_code == code:
            return band
    raise ValueError(f"Unknown Pan-STARRS1 filter code: {filter_code!r}.")
...
def adql_detection_lightcurve(*, obj_id: int, filter_type: str) -> str:
    """Builds ADQL for epoch photometry in one filter.

    Args:
        obj_id (int): Pan-STARRS mean object identifier.
        filter_type (str): ``Filter.filterType`` code (``g``, ``r``, …).

    Returns:
        str: Complete ADQL query string.
    """
    ftype = adql_escape_string(str(filter_type).strip().lower())
    return (
